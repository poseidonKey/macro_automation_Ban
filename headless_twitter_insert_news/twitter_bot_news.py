...
class NewsBot:
    ...
    def prepare_contents(self, keyword):
        ...
        news_elements = self.driver.find_elements(
            by=By.CLASS_NAME, value="SoaBEf")
        # 모든 엘레멘트로부터 정보를 추출하겠습니다.
        for el in news_elements:
            # 기사 제목을 추출합니다. <JheGif.nDgy9d> 태그로 작성되었습니다.
            headline = el.find_element(
                by=By.CLASS_NAME, value="mCBkyc.ynAwRc.MBeuO.nDgy9d").text
            # 기사 하이퍼링크 태그를 추출합니다.
            hyperlink = el.find_element(by=By.TAG_NAME, value="a")
            ...
            # 트윗에 올릴 기사 요약을 만듭니다.
            news_summary = "\n".join(
                (headline, self.endswith, news_url))
    ...
    def tweet(self, string):
        ...
        # board = self.driver.find_element_by_xpath(
